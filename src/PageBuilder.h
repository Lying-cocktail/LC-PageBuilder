...
//首先对URI_TYPE_SIGNATURE类型别名进行适配
#if defined(ARDUINO_ARCH_ESP8266) || (defined(ARDUINO_ARCH_ESP32) && (ESP_ARDUINO_VERSION < ESP_ARDUINO_VERSION_VAL(3, 0, 0)))
...
#endif // !PB_DEBUG_PORT
#ifdef PB_DEBUG
#define PB_DBG_DUMB(fmt, ...)                                \
  do                                                         \
  {                                                          \
    PB_DEBUG_PORT.printf_P((PGM_P)PSTR(fmt), ##__VA_ARGS__); \
  } while (0)
#define PB_DBG(fmt, ...)                                             \
  do                                                                 \
  {                                                                  \
    PB_DEBUG_PORT.printf_P((PGM_P)PSTR("[PB] " fmt), ##__VA_ARGS__); \
  } while (0)
#else
#define PB_DBG_DUMB(...) \
  do                     \
...
// and lacks completeness.
  ...
  using URI_TYPE_SIGNATURE = std::conditional<
      ...
      String>::type;
#else
  // william  SUPPORT ESP32 + Arduino 2.x
  using URI_TYPE_SIGNATURE = const String &;
#endif
...
//其次增加新版函数原型
  ...
  virtual bool  canHandle(WebServer &server,HTTPMethod requestMethod, PageBuilderUtil::URI_TYPE_SIGNATURE uri ) override;
